...
const Keyboard = ({ keys, keyOnClick }: Props) => {
  const mockEvent = (key: { char: string }) => {
    if (key.char === "Enter" || key.char === "Backspace")
      return { code: key.char, key: key.char, repeat: false };
    return {
      code: `Key${key.char.toUpperCase()}`,
      key: key.char,
      repeat: false,
    };
  };
      ...
      {keys.map((key) => (
        <Fragment key={v4()}>
          ...
          {(key.char === "p" || key.char === "l") && (
            <hr className="w-full -my-1.5 border-none" />
          )}
          {key.char === "l" && (
            <Key onClick={() => keyOnClick(mockEvent({ char: "Enter" }))}>
              Enter
            </Key>
          )}
          {key.char === "m" && (
            <Key onClick={() => keyOnClick(mockEvent({ char: "Backspace" }))}>
